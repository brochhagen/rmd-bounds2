...
def get_all_obs(k):
    ...
    all_obs = list(combinations_with_replacement(atomic_observations,k))
   
    obs = [] #store all produced k-length (s,m) sequences 
    for i in all_obs:
        obs.append(summarize_counts(i))
    return obs
...
#    out = np.dot(lhs,parametrized_post)
    ...
    return [out,normalize(out),lhs, obs]
