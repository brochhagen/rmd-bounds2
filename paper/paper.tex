...
\begin{abstract} 
Mutual reasoning endows speakers with the ability to convey information that goes beyond the literal meaning of expressions. A particularity of certain classes of lexical meanings is that they enable for such pragmatic enrichments in a notably productive fashion. This raises the challenge to justify their regular selection over alternatives that codify semantically what is conveyed pragmatically. That is, to justify cross-linguistically pervasive semantic structure in light of pragmatics. We provide an explanatory account for the roles of both domains by means of an analysis of the linguistic pressures that apply at the semantics-pragmatics interface. To this end, we propose a general model that combines functional pressure on language use and a learnability pressure in acquisition in populations of probabilistic language users with varied degrees of pragmatic sophistication. We apply this model in a case study on the (lack of) lexicalization of scalar implicatures. The results show that simpler semantic representations are selected for when languages are pressured towards learnability and compression, provided that pragmatic reasoning can compensate for the disadvantage in expressivity that users of such languages otherwise incur. We argue this result to shed light on the lack of lexicalization of scalar inferences, as well as the semantics-pragmatics distinction more generally.
\end{abstract}

\section{Introduction}\label{sec:introduction}
\hl{This should start more generally, stressing mutual reasoning and what it brings to the table.}
...
\hl{Mention similarity to Piantadosi+etal's prior}
...
\section{Conclusion}
Language change is affected by intertwined pressures. Drawing from past research, we put forward a general model that integrates central aspects involved in this process by combining cultural evolution dynamics with models of probabilistic language use. In the case of the lack of lexicalization of scalar implicatures, the model predicts that, when  pressured for learnability and expressivity, the former force drives for simpler semantic representations inasmuch as pragmatics can compensate for their lack of expressivity in use. That is, the relative learnability advantage of simpler semantics may offer an answer to why natural languages do not lexicalize certain pragmatic inferences. 

More broadly, we argued that semantic patterns can be explained by taking into consideration the way in which they are used in interactions as well as their representational complexity. As use more often than not concerns pragmatics, this kind of analysis not only offers an explanatory account for the phenomenon at hand, but sheds light on the semantics-pragmatics distinction by elucidating their respective roles in language change. 
