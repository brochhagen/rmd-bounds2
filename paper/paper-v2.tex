...
its simplest, discrete-time formulation, the RMD defines the frequencies $x'_i$ of each type
$i$ in a population at the next time step as a function of: (i) the frequencies $x_i$ of each
type $i$ before the update step, (ii) each the fitness $f_i$ of type $i$, and the probability
...
\begin{align}
  ...
  x'_i & = (\text{M}(\text{RD}(\vec{x})))_i \ \ \text{, where} \\
...
thus conceived: replication then mutation; if $f_i$ is the same for all $i$ we get only
multiplication by $Q$, if $Q_{ji} = 1$ exactly when $i=j$, we get the RD, la di da

main question what are types, what is fitness, what is $Q$ answered next
