...
The space of lexica we consider is given in Table \ref{tab:lexica}. These $(2,2)$-Boolean matrices are the simplest ones that allow us to make the contrast between the presence or abscence of an upper-bound and the use of scalar implicatures precise. One may think of the state corresponding to the first row of a matrix as a ``some but not all''-state and the second as an ``all''-state. The literal meaning of weak scalar expressions such as English {\em some} then corresponds to a message true of both rows and rows in these fragments. While there are $16$ possible $(2,2)$-lexica, a number of them are identical both in terms of expressivity and the learning bias. The competition between such types is determined by the initial configuration of a population -- the proportion with which each type starts out. However, this fact can be obscured when averaging across simulations. Simulations conducted with the full space of possible lexica confirm that the results reported here do not hinge on their exclusion. We therefore focus on this smaller representative subset. Lexica $L_1$ to $L_3$ are not optimal for communication because they assign the same state to all their messages. This failure to associate a single form to a meaning both semantically and pragmatically inevitably leads to their disadvantage in language use. $L_4$ and $L_5$ are our target lexica. They codify upper-bounded semantics for the message corresponding to the first matrix's column and a lack thereof. Lastly, $L_6$ is similar to $L_5$ in that two messages are true of the same state but differs from it in assigning upper-bounded semantics to the first column's message. 
...
Following our assumption of a preference for simple lexical representations, the prior biases learners against lexica in which a message holds true only of the first row. That is, those messages that lexicalize an upper-bound that rules out the ``all''-state. All other semantics are assumed to be a priori equally probable. Accordingly, this prior is captured by $P(t_i) \propto n - c \cdot r$, where $n$ is the total number of states and $r$ is the number of messages only true of row $1$ in $t_i$'s lexicon, $c \in [0,1]$. Increments in the prior parameter $c$ therefore bring about a stronger bias against languages that lexicalize upper-bounds, i.e., $L_2, L_4$ and $L_6$.
...
The assumption of a learning bias was pivotal in introducing transmission perturbations that differentiate similar types and homogenize the population. However, such perturbations need not necessarily arise from cognitive factors. Instead, as shown in the following, similar outcomes can arise from environmental actors, such as perceptual errors during production and comprehension. We should stress that our goal is not to suggest noisy perception to underlie the selection of a lack of semantic upper-bounds in scalar implicatures, but to elucidate the role of transmission perturbations in this process and the cultural evolution of meaning more broadly. 

The core components of the model remain as before. All that is required is an adjustment to the possibility of confusing one state of affairs with another. Letting $\epsilon$ stand for the probability of confusing the first row state with the second, and vice-versa for $\delta$, we denote the probability that the teacher (learner) observes state $s_t$ ($s_l$) when the actual state is $s_a$ as $P_N(s_t \mid s_a)$ ($P_N(s_l \mid s_a)$). The probability that $s_a$ is the actual state when the learner observes $s_l$ is just Bayes rule, combining prior of $s_a$ with the noise from above:
\begin{align*}
...
\paragraph{Results.} As shown in Table \ref{tab:num-noise}, a prevalence of pragmatic $L_5$ can also arise from noisy transmission without a prior  learning bias for particular types. In contrast to the outcome of the non-noisy model, favourable outcomes for this type concentrate in noise patterns with $\epsilon > \delta$, $\alpha \geq 5$, $1 < k < 20$ and $l > 3$. Crucially, these results show that the mere presence of systematic noise in the transmission of strategies can introduce regularization that looks as if the agents have a learning bias. But, most importantly, this disturbance of transmission fidelity is due to perceptual noise and properties of the environment, not learning biases. In other words, learning biases are clearly not the only transmission biases that can shape evolution alongside functional pressure. Environmental and perceptual noise can play a role too.

\begin{table}
\centering 
\begin{tabular}{l | c c c c c| c c c c c c c c}
\multicolumn{1}{c}{~} & \multicolumn{5}{c}{$l = $} & ~ & \multicolumn{5}{c}{$l = 15$}\\ \hline \hline
