...
from above would then assign $\ssome$ a probability of one after hearing $\msome$, while a
literal interpreter with lexicon $L_5$ would assign $\ssome$ probability $.5$. The simplest
definition of a literal speaker would just be $S_{0}(m|s;L) \propto \exp((L_{sm})^\alpha)$, but
the more complex formulation in (\ref{s:level0}) 

\begin{itemize}
\item no prob zero
\item $\alpha$ adjusts the level of avoidance of untrue signaling
\end{itemize}

 The behavior of literal speakers, given in
(\ref{s:level0}), is regulated by a parameter $\alpha$ which controls the sensitivity to which
speakers prefer one signal over another based on its expected communicative success.
