...
community made up of different types of agents. The dynamic assumes a virtually infinite
population, so that it is able to abstract away from what individual agents do and how they
change their individual behavior over time. The result is an abstract description of the most
probable trajectory of change in the population vector $\vec{x}$, which codifies relative
frequencies of types, so that $x_i$ is the relative frequency of type $i$ and
...
whenever $j=i$, the dynamic reduces to the replicator dynamic. If fitness
$f_i$ is the same for all types $i$, the replicator step is the identity map
$ \left ( \text{RD}(\vec{x}) \right )_i = x_i$ and the replicator mutator dynamics reduces to a
process of iterated mutation. Let us have a look at each component in isolation: first
replication, then mutation.
...
population-level changes when the number of offspring in single-parent replication is
...
better the behavior $j$, the more likely it is to be imitated.

A crucial assumption in all of the derivations of the replicator dynamic as a biological or
...
methodological choice. It allows us to study ---as the formal thought experiment that any such
model is--- whether whatever we would like to explain could have evolved also in the absence of
specific strategic or optimal agent-level reasoning. In other words, given its versatile
interpretability, the replicator dynamic is an abstract, austere and maximally neutral
description of how many specific kinds of global fitness-based selection play out in large
populations of agents who are not themselves optimizing their behavior strategically.

If all types have equal fitness no matter what there is no functional pressure and the
replicator mutator dynamic reduces to a process of iterated mutation, the bias of which is
encoded in mutation matrix $Q$. Mutation can be interpreted as biological or cultural as
well. Under a biological interpretation where types are genetically encoded behavioral traits,
$Q_{ij}$ is the probability by which offspring of type $i$ genetically mutates into type
$j$. Under a cultural interpretation where types are transmitted by observation or learning,
such as in conditional imitation, $Q_{ij}$ is the probability that an agent trying to learn,
adopt or imitate the behavior of an agent with type $i$ acquires type $j$. As detailed below,
...
A biological or cultural interpretation with either of the replicator or mutator component need
not necessarily imply the same kind of interpretation for the other. In the applications to
language evolution here, we clearly prefer a cultural interpretation of the mutator component
as the perturbation induces by learnability of linguistic traits. Nothing much hinges on
whether the replicator component is interpreted as a biological process or a cultural
process. Fitness-based selection may either be thought of as the outcome of actual reproductive
success (increased number of offspring which learn a language from their single parent), or, as
we slightly prefer here, as a process of cultural evolution, such as conditional imitation for
instance.
