...
An elegant formal approach to capture the interaction between selection gor communicative
...
is able to abstract away from what individual agents do. The result is an abstract description
of the most probable trajectory of change in the population vector $\vec{x}$, which codifies
relative frequencies of types, so that $x_i$ is the relative frequency of type $i$ and
$\sum\nolimits_i x_i = 1$. In its simplest discrete-time formulation, the RMD defines the
frequency $x'_i$ of each type $i$ in the population at the next time step as a function of: (i)
the frequency $x_i$ of each type $i$ before the update step, (ii) the fitness $f_i$ of each
type $i$ before the update, and (iii) the probability $Q_{ji}$ that an agent who observes the
behavior of an agent with type $j$ ends up acquiring type $i$:
\begin{align}
...
n the context of language evolution, fitness is usually associated with the ability to
...
$i$ when observing the behavior of a type-$j$ agent. Fitness-based can be thought of as
biological (fitness as expected relative number of offspring) or cultural evolution (fitness as
likelihood of being imitated or repeated). A type's communicative success depends on how well
it communicates within a population while its learnability depends on the fidelity by which it
is inferred by new generations of learners. The learners' task is consequently to perform a
joint inference over types of linguistic behavior and lexical meaning.
...
consider what are perhaps two of the simplest non-trivial setups that speak to this matter and
reflect on their limitations in Section~\ref{sec:discussion}. Section \ref{subsec:example}
first considers a minimal example intended to isolate effects of replication and mutation and
to gain insights into the sense in which semantic meaning and pragmatic use can be said to
co-evolve. We then turn to a larger and more realistic setup in
...
ultimately depends on the rationality parameter $\lambda$. Functional differences decrease as
$\lambda$ increases: if $\lambda$ is sufficiently high, all types except for literal $\Llack$
speakers will show a strong tendency to associate $\ssome$ with $\msome$ and $\sall$ with
$\mall$.
...
space. This is suggested by the solid purple arrows in Fig.~\ref{fig:sketch}. \mf{Should we
  include here a sentence on why the top-most arrow for biased learning points to pragmatic
  L-lack?} Taking stock, the effects of iterated learning will depend not only on $\lambda$,
affecting the likelihood, but also on the prior (how strong of a bias there is) and the degree
to which learners tend to maximize the posterior (controlled by parameter $l$).
...
better showcase their effects. How exactly these forces shape a population over time also
hinges on which types are present. Especially the effects of learning biases affected by
this. If, hypothetically, we add many functionally dispreferred types which all happen to
mutate with a high probability to the same type, this can strongly bias the dynamics in favor
of this type. \mf{maybe a sentence here about the top-most arrow in Figure 1?} For this reason,
we turn to a larger setup next with a larger and more realistic set of types and a suggestion
for how to formalize potential learning biases in lexical inferences.
...
enrichment can co-evolve.


\subsection{Setup}
\label{sec:setup:-stat-conc}

The effects of learning biases can depend on the set of types. We therefore consider next
larger and more realistic sets of states, messages and lexica. Additionally, we want to explore
