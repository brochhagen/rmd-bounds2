...
Table~\ref{tab:concepts} together with their complexity according to the grammar given in \ref{tab:grammar} and $Compl(c)$. 
...
\tb{Maybe note somewhere that pragmatic $\Lbound$ is functionally worse than literal/pragmatic $\Lbound$ but still very hard to distinguish when there's little data. But the latter has a learning advantage. Or do this while discussing each dynamic. See commented out below}
