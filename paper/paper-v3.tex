...
messages there are $6^3 = 216$ possible lexica. Of these, three kinds of lexica are of particular relevance to our purpose. First, lexica that assign the same concept to more than message. Such lexica lack in expressivity under either signaling behavior but may be easier to acquire and be favored by the learning bias. Second, lexica that conventionalize upper-bounds to realize a (quasi-)partition of the relevant semantic space. As discussed above in relation with $\Lbound$, users of these lexica need not resort to pragmatic reasoning to convey an upper-bound with weak scalar expressions. Instead, this information is already given by the semantics of their lexicon, giving them an advantage in terms of expressivity. Third and lastly, $\Llack$-style lexica that, paired with mutual reasoning, can be used to convey an upper-bound pragmatically. Overall, there are $6$ lexica of the second kind and $6$ of the third. The following three lexica exemplify each kind, with $\Lall$ being a lexicon that is not optimal for communication as it assigns all its messages to the same state $\sall$. This failure to be able to associate a state to single a form inevitably leads to a communicative disadvantage in its use. However, such a lexicon may be easy to acquire.
...
\caption{Proportion of pragmatic $\Llack$-style types and of the incumbent type in $5$ independent populations after $50$ generations under only a pressure for expressivity.}
\label{fig:only-R}
\end{figure}

The replicator dynamic is sensitive to $\lambda$ as it influences signaling behavior and thereby has a bearing on a type's fitness as well. This influence is showcased in Figure \ref{fig:only-R}, which shows the proportion of pragmatic $\Llack$-style speakers and that of the type with the highest proportion, the incumbent, in $5$ independent populations. As is to be expected, less expressive types do not fare well under pressure for communicative success and those able to convey states unequivocally fare better. This difference increases with $\lambda$ as it promotes more deterministic linguistic behavior, widening the gap in expressivity between types. However, higher values of $\lambda$ alone do not lend a justification to the systematic prevalence of $\Llack$-like semantics paired with pragmatic reasoning. An inspection of $1000$ independent populations with $\lambda = 20$ shows a similar outcome to that hinted at by Figure \ref{fig:only-R}. Pragmatic $\Llack$ types only make up $11$ cases of incumbency, corresponding to a mean proportion of $0.003$ across populations. By contrast, $913$ incumbents are $\Lbound$-style users with close to an even share between literal ($454$) and pragmatic types ($459$), corresponding to a mean proportion of about $48\%$. Overall, incumbents of any type make up $50\%$ of the mean outcome, showcasing the strong dominance of $\Lbound$ semantics over other alternatives when only expressivity is at stake. 
