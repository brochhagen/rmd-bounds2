...
cost = 0.5 # cost for LOT-concept with upper bound
...
def get_mutation_matrix_map(k):
    ...
    map_q = np.zeros((r,c))
    for i in xrange(r):
	r_max = np.random.choice(np.argwhere(q[i] == np.amax(q[i])).flatten(),1)[0]
        map_q[i,r_max] = 1
    return map_q
...
u = get_utils()
u = np.ones([len(typeList), len(typeList)])
