...
sample_amount = 10000 #amount of samples from OBS when k > 4
sample_with_replacement = False #To fix sampling problems. Set false to sample w/o replacement
...
f_unwgh_mean = csv.writer(open('./results/2multiscalar-unwgh-mean-a%d-c%f-l%d-k%d-g%d-r%d-s%d.csv' %(alpha,cost,lam,k,gens,runs,sample_with_replacement),'wb')) #file to store each unweighted simulation run after n generations
f_wgh_mean = csv.writer(open('./results/2multiscalar-wgh-mean-a%d-c%f-l%d-k%d-g%d-r%d-s%d.csv' %(alpha,cost,lam,k,gens,runs,sample_with_replacement),'wb')) #file to store each weighted simulation run after n generations

f_q = csv.writer(open('./results/2multiscalar-q-matrix-a%d-c%f-l%d-k%d-g%d-r%d-s%d.csv' %(alpha,cost,lam,k,gens,runs,sample_with_replacement),'wb')) #file to store Q-matrix
...
def get_obs(k):
    ...
    else: #producing the entire list crashes memory for k>9. So we sample indices insead. This is still taxing for large k
        if sample_with_replacement:
            D = [(choice(inputx),choice(outputy)) for _ in xrange(sample_amount)]
        else:
