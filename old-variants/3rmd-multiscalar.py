...
##Sample n states from the set of all states, pair with n messages
#####
...
sample_amount = 10000 #amount of samples
...
f_unwgh_mean = csv.writer(open('./results/3multiscalar-unwgh-mean-a%d-c%f-l%d-k%d-g%d-r%d.csv' %(alpha,cost,lam,k,gens,runs),'wb')) #file to store each unweighted simulation run after n generations
f_wgh_mean = csv.writer(open('./results/3multiscalar-wgh-mean-a%d-c%f-l%d-k%d-g%d-r%d.csv' %(alpha,cost,lam,k,gens,runs),'wb')) #file to store each weighted simulation run after n generations

f_q = csv.writer(open('./results/3multiscalar-q-matrix-a%d-c%f-l%d-k%d-g%d-r%d.csv' %(alpha,cost,lam,k,gens,runs),'wb')) #file to store Q-matrix
...
def get_obs(k):
    ...
    inputx = [np.random.random_integers(0,high=(states*lexical_pairs -1),size=(k)) for _ in xrange(sample_amount)] #k-tuple where the i-th observed state was state k_i 
    outputy = [np.random.random_integers(0,high=(messages -1),size=(k)) for _ in xrange(sample_amount)] #k-tuple where the i-th observed message was k_i 
    return summarize_counts(zip(inputx,outputy),k)
