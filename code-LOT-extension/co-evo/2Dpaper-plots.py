...
def quiver_contour(lam_lst,bias_para,post_para_lst,k,cont):
    lexica = [np.array([[1.,0.],[0.,1.]]),np.array([[1.,0.],[1.,1.]])]
    fig,axs = plt.subplots(ncols=3,nrows=2,sharex=True,sharey=True)#,sharey=True,sharex=True)
...
lam_lst = [1,5,20,1,5,20]
post_para_lst = [1,10,20,1,10,20]

quiver_contour(lam_lst,1.05,post_para_lst,5,True) #If True then contour, otherwise no contour background
